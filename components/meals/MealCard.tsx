        ...
        <div className="flex items-start justify-between gap-1">
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 mb-0.5">
              <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${MEAL_COLORS[meal.mealType] || "bg-slate-100 text-slate-600"}`}>
          ...
          {onDelete && (
            <button onClick={() => onDelete(meal.id)} className="shrink-0 text-slate-300 hover:text-red-400 transition-colors p-1">
              <Trash2 size={16} />
            </button>
          )}
        </div>
